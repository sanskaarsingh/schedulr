    ...
    <div className="p-4 bg-white rounded-lg shadow">
      ...
      <div className="grid grid-cols-7 gap-1 mt-2">
        {daysInGrid.map((day, index) => (
          <div key={index} className={`border rounded p-2 h-40 overflow-y-auto ${day ? 'bg-white' : 'bg-gray-50'}`}>
            {day && (
              <>
                <div className={`text-sm font-bold ${DateTime.fromJSDate(day).hasSame(DateTime.now(), 'day') ? 'text-indigo-600' : 'text-gray-700'}`}>
